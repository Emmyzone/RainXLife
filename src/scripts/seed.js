...
const category = db.prepare('SELECT * FROM categories WHERE slug = ?').get('money-and-wealth');

const existing = db.prepare('SELECT * FROM articles WHERE slug = ?').get('atomic-habits-summary');
if (existing) {
